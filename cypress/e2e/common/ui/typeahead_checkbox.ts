...
const typeaheadCheckboxToggle = (dataCy: string) =>
  cy.dataCy(`${dataCy}-multi-typeahead-checkbox-menu-toggle`);
...
const typeaheadCheckboxOption = (option: string, dataCy: string) =>
  cy
    .get(
      `[data-cy='select-attrs-${option}'], [data-cy='${dataCy}-${option}'], [data-cy='${dataCy}-${option}-create-new-option']`
    )
    .first();
...
export const openTypeaheadCheckboxMenu = (dataCy: string) => {
  typeaheadCheckboxToggle(dataCy).then(($toggle) => {
    if (!$toggle.hasClass("pf-m-expanded")) {
      cy.wrap($toggle).find('[role="combobox"]').click();
    }
  });
  typeaheadCheckboxToggle(dataCy).should("have.class", "pf-m-expanded");
};
